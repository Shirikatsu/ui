...
const getFormat = memoize_1(format => ['y', 'M', 'd', 'h', 'm', 's'].reduce((units, symbol) => formats[format].includes(symbol) ? [...units, unitNames[symbol]] : units, []));

function getTime(start, end, format, showEmpty, maxUnits) {
  const date1 = end || new Date();
  const date2 = end ? new Date() : start;
  const totalInSeconds = dayjs_min(date1).diff(date2, 'seconds');
  const {
    years,
    months,
    days,
